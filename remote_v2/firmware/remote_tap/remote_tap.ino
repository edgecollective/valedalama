...
#define tapper 9
#define tap_delay 50 // in milliseconds

SerialCommand sCmd(Serial);         // The demo SerialCommand object, initialize with any Stream object

void setup() {
  pinMode(arduinoLED, OUTPUT);      // Configure the onboard LED for output
  pinMode(tapper, OUTPUT);      // Configure the onboard LED for output

  digitalWrite(arduinoLED, LOW);    // default to LED off
  digitalWrite(tapper, HIGH);    // default to LED off
  ...
  sCmd.addCommand("P",     processCommand);  // Converts two arguments to integers and echos them back
    sCmd.addCommand("RESET", resetter);        // Echos the string argument back
  sCmd.addCommand("BOOT", booter);        // Echos the string argument back
...
void resetter(SerialCommand this_sCmd) {
  this_sCmd.println("RESET");
  digitalWrite(arduinoLED, HIGH);
  digitalWrite(tapper, LOW);
  delay(tap_delay);
  digitalWrite(arduinoLED, LOW);
  digitalWrite(tapper, HIGH);

}

void booter(SerialCommand this_sCmd) {
